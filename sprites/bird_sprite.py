...
class Bird(arcade.Sprite):
    def __init__(self, x, y):
        ...
        self.textures = []
        texture = arcade.load_texture("images/bird1.png")
        self.textures.append(texture)
        texture = arcade.load_texture("images/bird2.png")
        self.textures.append(texture)
    ...
    def update(self, delta_time: float = 1 / 60, player_coords=None):
        ...
        if abs(self.center_x - player_x) < abs(self.center_y - player_y):
            self.change_y = player_y - self.center_y
            self.change_x = 0
        else:
            self.change_x = player_x - self.center_x
            self.change_y = 0
        if self.change_x > 0:
